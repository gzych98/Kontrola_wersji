import os
import shutil
from PyQt5.QtWidgets import QTextEdit, QListWidgetItem, QMainWindow, QPushButton, QLabel, QListWidget, QVBoxLayout, QHBoxLayout, QWidget, QDesktopWidget, QLineEdit, QDialog, QSystemTrayIcon, QMenu, QAction
...
class MainApp(QMainWindow):
    ...
    def initUI(self):
       ...
       # self.listWidget = DragDropListWidget(self)
        ...
        self.integration_button.clicked.connect(
            lambda: self.run_app_process(["--integration", "--file"]))

        self.measurement_button = QPushButton("Measurement", self)
        self.measurement_button.clicked.connect(
            lambda: self.run_app_process(["--measurement", "--file"]))

        self.standalone_button = QPushButton("Standalone", self)
        self.standalone_button.clicked.connect(
            lambda: self.run_app_process(["--gen-standalone", "--input-model"]))
        ...
        self.solver_path_edit.setText(
            r"C:\Program Files\Simpack-2023x.3\run\bin\win64\simpack-slv")

        self.pre_path_edit = QLineEdit(self)
        self.pre_path_edit.setPlaceholderText("Wprowadź ścieżkę do pre")
        self.pre_path_edit.setText(
            r"C:\Program Files\Simpack-2023x.3\run\bin\win64\simpack-gui")
    ...
    def copy_model(self):
        ...
        for item in selected_items:
            ...
            timestamp = datetime.datetime.now().strftime(
                "%Y-%m-%d_%H-%M-%S__")  # Formatowanie znacznika czasu
            # Oddzielenie nazwy pliku od ścieżki
            directory, filename = os.path.split(full_path)
            # Dodanie znacznika czasu do nazwy pliku
            new_filename = f"{timestamp}_{filename}"
            # Nowa ścieżka z uwzględnieniem znacznika czasu
    ...
    def otworz_w_simpack_pre(self):
        ...
        if selected_items:
            ...
            process_args = []
            aktywuj_simpack_pre_i_otworz_plik(
                self.listbox, self.info_label, self, process_args, pre_path)
        else:
    ...
    def hide_window(self):
        ...
        self.tray_icon.showMessage(
            "Informacja", "Aplikacja została zminimalizowana do zasobnika systemowego.")

    def clear_list(self):
        # Funkcja czyszcząca listę
        self.listbox.clear()

# Możesz również zdefiniować inne klasy, jeśli są potrzebne
